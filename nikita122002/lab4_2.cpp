...
int main()
{
	...
	FILE* stream2;
	errno_t err;
	err = fopen_s(&stream1, "code.txt", "r");//open file for reading
	err = fopen_s(&stream2, "keywords.txt", "r");
	fgets(keywords, len2, stream2);//read symbols and save as raw
	int i;
	bool isFound;
	bool isEmpty;
		...
		while (code[i] != '\0') //reads words to the end of the line
		{
			...
			for (int j = 0; ; j++, i++) //reads a word
			{
				if (code[i] == ' ' || code[i] == '{' || code[i] == '}' || code[i] == '(' || code[i] == ')' || code[i] == '.'
					...
					|| code[i] == '#' || code[i] == '<' || code[i] == '>' || code[i] == '\0' || code[i] == '\t' ) 
				{
					break;
				}
				buff[j] = code[i];
