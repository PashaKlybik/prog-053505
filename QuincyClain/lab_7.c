...
struct _Code {
 ...
 struct _Comments {       // Суть в том, что каждая информационная ячейка двусвязного списка хранит 3 раздела: Сам код, тайтл, в котором написано на каком языке выполнено + тема кода + дата добавления			
	char comment[1000];	  // Функционал включает поиск исходника по языку прогг, по теме кода и так же вывод всех доступных исходников. Так же пользователь может добавлять комментарии тем самым изменяя ячейку comments и редактировать их.
};
 struct Node {
	_Code code;
	_Title title;
...
void output(Node* head) {  // вывод всех
	do {
		printf("\nИсходник\n");
...
void one_output(Node* head){ //вывод одного после поиска
		printf("\nИсходник\n");
		printf("%s", head->code.code);
		printf("\nСтатья\n");
		printf("Язык: %s", head->title.Language);
...
void addComments(Node* head){ //добавление комментария к разделу
	char string[1000];
	char prog[50];
...
void free_all(Node** h, Node** t) {  // очистка памяти
	Node* s;
	do {
		s = *h;
		(*h) = (*h)->prev;
...
int main()
{
	...
	free_all(&head, &tail); //очистка памяти!!!
	return 0;
}