...
// получаем любую выделяемую строку из файла 
char* get_string(FILE* file_or_con)
{
    char* str;
    int chinput;
...
/* получаем любой выделяемый текст из файла или stdin.
   остановка чтения:
   1) if max_line_num == 0 , то условие для чтения возращает 1 при вызове
   2) if max_line_num > 0 после чтения строки max_line_num  */
text* get_text(FILE* file_or_con, int (*condition_for_reading)(), int max_line_num)
{
    char** lines;
    char* line;
...
// Если оба параметра !=null , то записываем кол-во строк в файл 
void move_text_to_file(FILE* file, text* text)
{
