...
#pragma region output
    ...
    printf("%s ", (one->type == 0 ? "Композиция" : one->type == 1 ? "Сингл" :
        one->type == 2 ? "LP" : one->type == 3 ? "EP" : "Микс"));
    out_str_wo_nl(one->name);
    printf(" от ");
...
#pragma region user or group
    ...
    do
    {
        printf("Вы одиночный испольнитель или группа? \n");
        user_or_group = getch();
...
#pragma region option to add members if group
    ...
    if (user_or_group == 'g')
    {
        ...
        do
        {
            printf("\nХотите ли вы добавить участников в группу или сделаете это позже? \n");
            when = getch();
        } while (when != 'r' && when != 'l');
        ...
        if (when == 'r')
        {
            ...
            do
            {
                ...
                {
                    printf("Этот человек уже состоит в группе! ");
                }
                else
                {
                    ...
                    printf("Этот человек был добавлен в группу. ");
                }

                printf("Нажмите на n если этого достаточно: ");
                chinput = getch();
            } while (chinput != 'n');
        }
    }
...
#pragma region getting name and checking if it is unique
    ...
    {
        free(name);
        printf("\nУже есть человек с таким именем. Введите снова: ");
        name = get_string(stdin);
    }
...
#pragma region selecting
    ...
    while (1)
    {
        ...
        for (i = 0; i < num_of_available; i++)
        {
            ...
            printf(" %d\t\t %s: ", i + 1, index == 0 ? "Статья" : index == 1 ? "Группа" :
                index == 2 ? "Релиз" : index == 3 ? "Пользователь" : NULL);
        ...
        printf("\nНажмите на тег или на enter, чтобы выбрать : ");
        free(tag);
...
#pragma region options output
        ...
        printf("Нажмите:\t\tна:\n"
            ...
            " 4\t\t Купить запись\n"
            " 5\t\t Купить статью\n"
            " 6\t\t Смотреть мою запись\n"
            " 7\t\t Смотреть мою статью\n");

        if (group_you != NULL)
        {
            printf(" 8\t\t Добавить члена в группу\n"
                " 9\t\t Удалить члена группы\n");
        }
        printf(" 0\t\t Выйти\n");
...
#pragma region input processing
        ...
        if (chinput == '1') // Добавить денег
        {
            system("cls");
            printf("Введите сумму: ");
            add_money(you);
        }
        else if (chinput == '2') //Выпустить запись 
        {
            release_a_record(you);
        }
        else if (chinput == '3') //Опубликовать статью
        {
            publish_an_article(you);
        }
        else if (chinput == '4') // Купить запись 
        {
            buy_a_record(you);
        }
        else if (chinput == '5') //Купить статью
        {
            buy_an_article(you);
        }
        else if (chinput == '6') // Смотреть мою запись
        {
            int i = 0;
        ...
        else if (chinput == '7') // Смотреть мою статью
        {
            int i = 0;
            ...
            printf("\nНажмите на любую клавишу, чтобы вернуться к вашему аккаунту: ");
            getch();
        }
        else if (chinput == '8' && group_you != NULL) // Добавить члена
        {
            id new_member = select_one(1, 0, 0, 0);

            if (is_among(group_you->members, group_you->num_of_members, new_member))
            {
                printf("Этот пользователь уже состоит в группе. Нажмите на любую клавишу, чтобы продолжить: ");
                getch();
                continue;
            }
            ...
            printf("Этот пользователь был добавлен в группу. Нажмите на любую клавишу, чтобы продолжить: ");
            getch();

        }
        else if (chinput == '9' && group_you != NULL) // Удалить члена группы
        {
            if (group_you->num_of_members == 0)
            {
                printf("Никого не убрали из группы. Нажмите на любую клавишу, чтобы продолжить: ");
                getch();
                continue;
            }

            printf("Кого бы вы хотели удалить?\nВведите:\t\tУдалить:\n\n");
            printf(" 0\t\t Никого\n");
            int i = 0;
            for (; i < group_you->num_of_members; i++)
            {
            ...
            if (group_you->members == NULL)
                report_merror();
            printf("Этот пользователь был выгнан из группы. Нажмите на любую клавишу чтобы продолжить: ");
            getch();
...
#pragma region checking account for int overflow
    ...
    {
        printf("\nНевозможно добавить такую большую сумму. Нажмите на любую клавишу, чтобы продолжить: ");
        getch();
        return;
    }
...
#pragma region adding money
    ...
    printf("\n%d$ было добавлено на ваш аккаунт.Нажмите на любую клавишу, чтобы продолжить:", sum);
    getch();
    return;
...
#pragma region setting type
    ...
    printf("Это запись\n"
        "Композиция(1)\tСингл(2)\tLP(3)\tEP(4)\tМикс(5)? ");
...
#pragma region seting songs

    {
        ...
        //настройка номер песен
        do
        {
            printf("\nВведите номер песен а записи: ");
...
#pragma region releasing
    ...
    // добавление нового выпуска к вашему в качестве идентификатора
    ...
    //добавление нового выпуска ко всем из них 
...
#pragma region setting text the easy way
    ...
    // настройка кол-ва строк
    int lines_num;
    do
    {
        printf("Введите количество строк в статье: ");
        lines_num = get_int();
...
#pragma region publishing
    ...
    // добавление к вашему в качестве идентификатора
...
#pragma region checking if it is already yours
    ...
    if (is_among(user_you->purchased_releases, user_you->num_of_purchased_releases,
        selected->release_id))
    {
        printf("Он уже твой. Нажмите любую клавишу, чтобы вернуться в свою учетную запись");
        getch();
        return;
    }
...
#pragma region confirmation
    ...
    printf("\n%sХотите ли вы приобрести это за %d$? (y / n) ", selected->name, selected->price);
    int chinput;
    do
    {
        chinput = getch();
...
#pragma region checking affordableness and money transfer
    ...
    if (chinput == 'y')
    {
        ...
        {
            printf("\nУ вас больше нет денег. Нажмите любую клавишу, чтобы вернуться в свою учетную запись");
            getch();
            return;
        }
        else
        {
            ...
            printf("\nВы купили это. Нажмите любую клавишу, чтобы вернуться в свою учетную запись");
            getch();
            return;
        }
    }
...
#pragma region checking if it is already yours
    ...
    if (is_among(user_you->purchased_articles, user_you->num_of_purchased_articles,
        selected->article_id))
    {
        printf("Он уже ваш. Нажмите любую клавишу, чтобы вернуться в свою учетную запись");
        getch();
        return;
    }
...
#pragma region confirmation
    ...
    printf("Хотите ли вы купить ");
    out_str_wo_nl(selected->title);
    printf(" за %d$? (y / n)", selected->price);
    int chinput;
    do
    {
        chinput = getch();
...
#pragma region setting name, cover & music
    ...
    printf("\nУстановите обложку песни(0 для none): ");
    new_one.cover = get_int();
    printf("\nУстановите музыку (просто введите int):: ");
...
#pragma region setting lyrics the easy way
    ...
    do
    {
        printf("\nВведите количество строк в текстах: ");
        lines_num = get_int();
